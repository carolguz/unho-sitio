...
const UNHO = {
  ...
  // boton Eliminar: usa onclick inline (igual que los botones Agregar) Y data-* para la delegacion
  botonEliminar(tab, row) {
    return '<div class="li-actions"><button class="btn-del" type="button" data-tab="' + tab + '" data-row="' + row + '" ' +
      'onclick="UNHO.borrarFila(\'' + tab + '\',\'' + row + '\',this)">Eliminar</button></div>';
  },
  ...
  // ════════ PORTADA (texto + foto de fondo) ════════
  async renderPortada() {
    const rows = await this.leerTab('Portada');
    const p = (rows && rows[0]) ? rows[0] : {};
    const t = document.getElementById('inp-hero-texto');
    const d = document.getElementById('inp-hero-bg-drive');
    if (t) t.value = p.texto || '';
    if (d) d.value = p.imagenUrl || '';
  },
  async _guardarPortada() {
    const fila = {
      texto: this.val('inp-hero-texto'),
      imagenUrl: this.val('inp-hero-bg-drive')
    };
    return this.enviar({ accion: 'guardar', pestana: 'Portada', fila });
  },
  async saveHeroTexto() { await this._guardarPortada(); this.showMsg('msg-hero-texto'); },
  async saveHeroBg() { await this._guardarPortada(); this.showMsg('msg-hero-bg'); },
  previewHeroBg() {
    const v = this.val('inp-hero-bg-drive');
    if (!v) { alert('Pega el enlace o ID de Drive de la foto'); return; }
  ...
  async renderContacto() {
    const rows = await this.leerTab('Contacto');
    const c = (rows && rows[0]) ? rows[0] : {};
    const set = (id, v) => { const el = document.getElementById(id); if (el) el.value = v || ''; };
    set('inp-cont-direccion', c.direccion);
    set('inp-cont-telefono', c.telefono);
    set('inp-cont-correo', c.correo);
    set('inp-cont-cuenta', c.cuenta);
    set('inp-cont-banco', c.banco);
    set('inp-cont-facebook', c.facebook);
    set('inp-cont-tiktok', c.tiktok);
  },
  async saveContacto() {
    const fila = {
      direccion: this.val('inp-cont-direccion'),
      telefono: this.val('inp-cont-telefono'),
      correo: this.val('inp-cont-correo'),
      cuenta: this.val('inp-cont-cuenta'),
      banco: this.val('inp-cont-banco'),
      facebook: this.val('inp-cont-facebook'),
      tiktok: this.val('inp-cont-tiktok')
    };
    await this.enviar({ accion: 'guardar', pestana: 'Contacto', fila });
